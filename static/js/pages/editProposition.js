...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveProposalOptions = async () => {
        ...
        const response = await fetch('/com/api/proposals/options/edit/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCsrfToken(),
    ...
    if (addedProductsBody) {
        addedProductsBody.addEventListener('click', async (event) => {
            ...
            try {
                const response = await fetch('/com/api/proposals/remove-product/edit/', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': getCsrfToken(),
    ...
    if (previewLinks.length > 0) {
        previewLinks.forEach((linkElement) => {
            linkElement.addEventListener('click', async (event) => {
                ...
                const href = linkElement.getAttribute('href') || '/com/preview_proposition_page_edit/';
