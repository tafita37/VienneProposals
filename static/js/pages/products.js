...
document.addEventListener('DOMContentLoaded', () => {
	...
	const attachRowEvents = (row) => {
		...
		editButton.addEventListener('click', () => {
			handleEditProduct(
				...
				{
					id: editButton.dataset.productCategoryId,
					name: editButton.dataset.productCategoryName,
				},
				{
	...
	const createRow = (product) => {
		...
		row.innerHTML = `
			...
			<td style="padding: 0.75rem; border-bottom: 1px solid var(--border);">${escapeHtml(product.category_name)}</td>
			...
			<td style="padding: 0.75rem; border-bottom: 1px solid var(--border);">
				<button
					...
					data-product-category-id="${escapeHtml(product.category_id)}"
					data-product-category-name="${escapeHtml(product.category_name)}"
